...
async function fetchOutletData(outletId, url){
  ...
  if (salesSummaryTabName) {
    ...
    window.SALES_SUMMARY_DATA[outletId] = parsedTabs[salesSummaryTabName].map(function(row) {
      ...
      var pkg = cleanNum(row['Packaging'] || row['packaging'] || 0);
      var calculatedNet = gross - tax - pkg;
...
window.switchSalesTab = function(tab, btn) {
  ...
  if (slotFilter) {
    slotFilter.style.display = (tab === 'slots' || tab === 'datatables') ? 'inline-block' : 'none';
  }
...
window.runDayDrill = function() {
  ...
  var data = window.SALES_SUMMARY_DATA && window.SALES_SUMMARY_DATA[outletId];
  if (!data) return;
  ...
  var selectedWeekdayShort = dayNamesShort[selectedWeekday];

  var todayRow = data.find(function(r){ return r.Date === selectedDate && r.MealSlot.toUpperCase().indexOf('ALL DAY') !== -1; });
  if (!todayRow) { if(placeholder) placeholder.style.display='block'; return; }
  ...
  if(kpiGrid) {
    ...
    kpiGrid.innerHTML = [
      {l:'GROSS REVENUE', v:'₹'+fmtN(todayRow.GrossRevenue), s:dayNamesFull[selectedWeekday]+' · '+selectedDate, c:'var(--grn)'},
  ...
  // ── Date helpers ──
  var dateOffset = function(dateStr, days) {
    var d = new Date(dateStr + 'T00:00:00'); d.setDate(d.getDate()+days);
    return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');
  };
  var fmtRef = function(dateStr) {
    if (!dateStr) return '—';
    var d = new Date(dateStr + 'T00:00:00');
    return dayNamesShort[d.getDay()]+' '+d.getDate()+' '+mNames[d.getMonth()];
  };
  var getAllDayRow = function(dateStr) {
    return data.find(function(r){ return r.Date===dateStr && r.MealSlot.toUpperCase().indexOf('ALL DAY')!==-1; })||null;
  };
  var sameWeekdayRowsIn = function(monthStr) {
    return data.filter(function(r){
      return r.Date && r.Date.indexOf(monthStr)===0 && r.MealSlot.toUpperCase().indexOf('ALL DAY')!==-1 && getDayNameFromDate(r.Date)===selectedWeekdayShort && r.Date < selectedDate;
    }).sort(function(a,b){ return b.Date.localeCompare(a.Date); });
  };
  var avgOf = function(rows, field) {
    return rows.length ? rows.reduce(function(a,r){ return a+(r[field]||0); },0)/rows.length : null;
  };

  var selYear = selD.getFullYear(), selMonthIdx = selD.getMonth();
  var prevMonthIdx = selMonthIdx===0 ? 11 : selMonthIdx-1;
  var prevMonthYear = selMonthIdx===0 ? selYear-1 : selYear;
  var prevMonthStr  = prevMonthYear+'-'+String(prevMonthIdx+1).padStart(2,'0');
  var lyMonthStr    = (selYear-1)+'-'+String(selMonthIdx+1).padStart(2,'0');
  var lastYearDate  = (selYear-1)+'-'+String(selMonthIdx+1).padStart(2,'0')+'-'+String(selD.getDate()).padStart(2,'0');

  var lastWeekDate   = dateOffset(selectedDate, -7);
  var lastWeekRow    = getAllDayRow(lastWeekDate);
  var prevMoWdRows   = sameWeekdayRowsIn(prevMonthStr);
  var prevMoSameWdRow = prevMoWdRows[0]||null;
  var prevMoAvgGross = avgOf(prevMoWdRows.slice(0,4), 'GrossRevenue');
  var prevMoAvgOrd   = avgOf(prevMoWdRows.slice(0,4), 'TotalOrders');
  var last4SameWd    = data.filter(function(r){
    return r.Date && r.MealSlot.toUpperCase().indexOf('ALL DAY')!==-1 && getDayNameFromDate(r.Date)===selectedWeekdayShort && r.Date < selectedDate;
  }).sort(function(a,b){ return b.Date.localeCompare(a.Date); }).slice(0,4);
  var avg4Gross = avgOf(last4SameWd, 'GrossRevenue');
  var avg4Ord   = avgOf(last4SameWd, 'TotalOrders');
  var lastYearRow  = getAllDayRow(lastYearDate);
  var lyMoWdRows   = sameWeekdayRowsIn(lyMonthStr);
  var lyAvgGross   = avgOf(lyMoWdRows, 'GrossRevenue');
  var lyAvgOrd     = avgOf(lyMoWdRows, 'TotalOrders');

  // ── Comparison Table ──
  if(compCard) {
    compCard.style.display = 'block';
    var mkRow = function(label, refGross, refOrders, refLabel) {
      if (refGross===null||refGross===undefined) {
        return '<tr><td style="font-weight:600;color:var(--m2)">'+label+'</td><td style="color:var(--m1);font-size:11px">'+refLabel+'</td><td colspan="6" class="num" style="color:var(--m1);font-style:italic">No data available</td></tr>';
      }
      var rev=todayRow.GrossRevenue, ord=todayRow.TotalOrders;
      var rd=rev-refGross, od=ord-(refOrders||0);
      var rp=refGross>0?(rd/refGross*100):0;
      var down=rd<0;
      var col=down?'var(--red)':'var(--grn)';
      var sign=rd>=0?'+':'';
      var sig='<span style="font-size:11px;font-weight:700;padding:2px 6px;border-radius:4px;background:'+(down?'rgba(244,63,94,0.12)':'rgba(16,185,129,0.12)')+';color:'+col+'">'+(down?'🔴 DROP':'🟢 GROWTH')+'</span>';
      return '<tr>'
        ...
        +'<td class="num" style="color:'+col+';font-weight:700">'+sign+'₹'+fmtN(Math.abs(rd))+(down?' ▼':' ▲')+'</td>'
        +'<td class="num" style="color:'+(od>=0?'var(--grn)':'var(--red)')+'">'+( od>=0?'+':'')+od.toFixed(0)+'</td>'
        +'<td class="num" style="color:'+col+';font-weight:700">'+sign+rp.toFixed(1)+'%</td>'
        +'<td class="num">'+sig+'</td>'
        +'</tr>';
    };
    document.getElementById('drillCompBody').innerHTML = [
      mkRow('Same Day Last Week', lastWeekRow?lastWeekRow.GrossRevenue:null, lastWeekRow?lastWeekRow.TotalOrders:null, fmtRef(lastWeekDate)),
      mkRow('Same '+selectedWeekdayShort+' Last Month', prevMoSameWdRow?prevMoSameWdRow.GrossRevenue:null, prevMoSameWdRow?prevMoSameWdRow.TotalOrders:null, prevMoSameWdRow?fmtRef(prevMoSameWdRow.Date):mNames[prevMonthIdx]+' (none found)'),
      mkRow('Avg '+selectedWeekdayShort+'s Last Month', prevMoAvgGross, prevMoAvgOrd, prevMoWdRows.slice(0,4).length+' '+selectedWeekdayShort+'s in '+mNames[prevMonthIdx]),
      mkRow('4-Week Same Weekday Avg', avg4Gross, avg4Ord, last4SameWd.length+' pts: '+(last4SameWd.length?last4SameWd.map(function(r){return fmtRef(r.Date);}).join(', '):'none')),
      mkRow('Same Date Last Year', lastYearRow?lastYearRow.GrossRevenue:null, lastYearRow?lastYearRow.TotalOrders:null, fmtRef(lastYearDate)+' '+(selYear-1)),
      mkRow('Avg '+selectedWeekdayShort+'s Last Year Same Mo', lyAvgGross, lyAvgOrd, lyMoWdRows.length+' pts in '+mNames[selMonthIdx]+' '+(selYear-1)),
    ].join('');
  }
  ...
  if(channelCard) {
    ...
    var refRow = prevMoSameWdRow;
    var channels = [
      {n:'Dine-In',        icon:'🍽️', today:todayRow.DineInRev,  ref:refRow?refRow.DineInRev:null},
      {n:'Pickup/Takeaway',icon:'🛍️', today:todayRow.PickupRev,   ref:refRow?refRow.PickupRev:null},
      {n:'Delivery (All)', icon:'🛵', today:todayRow.DeliveryRev, ref:refRow?refRow.DeliveryRev:null},
      {n:'Zomato',         icon:'🍅', today:todayRow.ZomatoRev,   ref:refRow?refRow.ZomatoRev:null},
      {n:'Swiggy',         icon:'🧡', today:todayRow.SwiggyRev,   ref:refRow?refRow.SwiggyRev:null},
    ];
    document.getElementById('drillChannelViz').innerHTML = channels.map(function(ch){
      var delta=ch.ref!==null?(ch.today-ch.ref):null;
      var pct=(delta!==null&&ch.ref>0)?(delta/ch.ref*100):null;
      var col=delta===null?'var(--m1)':delta>=0?'var(--grn)':'var(--red)';
      ...
      return '<div style="background:var(--s2);border:1px solid var(--b1);border-radius:12px;padding:12px;text-align:center">'
        ...
        +(pct!==null?'<div style="font-size:11px;color:'+col+';font-weight:700;margin-top:4px">'+sign+pct.toFixed(1)+'%</div>':'<div style="font-size:10px;color:var(--m1);margin-top:4px">No ref</div>')
        +'</div>';
    }).join('');
    document.getElementById('drillChannelBody').innerHTML = channels.map(function(ch){
      if(ch.ref===null) return '<tr><td>'+ch.icon+' '+ch.n+'</td><td class="num">₹'+fmtN(ch.today)+'</td><td class="num" colspan="3" style="color:var(--m1);font-style:italic">No reference data</td><td class="num">—</td></tr>';
      var delta=ch.today-ch.ref, pct=ch.ref>0?(delta/ch.ref*100):0, down=delta<0;
      var col=down?'var(--red)':'var(--grn)';
      var verdict=down?(Math.abs(pct)>15?'🔴 Major Drop':'🟠 Minor Drop'):(pct>10?'🟢 Strong Gain':'🔵 Stable');
      return '<tr>'
        +'<td style="font-weight:600">'+ch.icon+' '+ch.n+'</td>'
        +'<td class="num">₹'+fmtN(ch.today)+'</td>'
        +'<td class="num">₹'+fmtN(ch.ref)+'</td>'
        +'<td class="num" style="color:'+col+';font-weight:700">'+(delta>=0?'+':'')+'₹'+fmtN(Math.abs(delta))+'</td>'
        +'<td class="num" style="color:'+col+';font-weight:700">'+(delta>=0?'+':'')+pct.toFixed(1)+'%</td>'
  ...
  if(slotCard) {
    ...
    var refSlotDate=prevMoSameWdRow?prevMoSameWdRow.Date:null;
    var refSlotRows={};
    slotNames.forEach(function(s){ refSlotRows[s]=refSlotDate?data.find(function(r){ return r.Date===refSlotDate&&r.MealSlot===s; })||null:null; });

    killChart('chDrillSlot');
    var cc=document.getElementById('chartDrillSlot');
    if(cc){
      CI.chDrillSlot=new Chart(cc,{
        type:'bar',
        data:{
        ...
        options:{responsive:true,maintainAspectRatio:false,
          scales:{x:{grid:{color:GC}},y:{grid:{color:GC},ticks:{callback:function(v){ return '₹'+fmtN(v); }}}},
    ...
    document.getElementById('drillSlotBody').innerHTML = slotNames.map(function(s,idx){
      var ts=todaySlotRows[s], rs=refSlotRows[s];
      var tRev=ts?ts.GrossRevenue:0, tOrd=ts?ts.TotalOrders:0;
      var rRev=rs?rs.GrossRevenue:null, rOrd=rs?rs.TotalOrders:null;
      var rd=rRev!==null?(tRev-rRev):null, od=rOrd!==null?(tOrd-rOrd):null;
      var rp=(rd!==null&&rRev>0)?(rd/rRev*100):null;
      var down=rd!==null&&rd<0;
      var col=rd===null?'var(--m1)':down?'var(--red)':'var(--grn)';
      var dineShare=tRev>0&&ts?((ts.DineInRev||0)/tRev*100).toFixed(0)+'%D':'—';
      var delShare=tRev>0&&ts?(((ts.ZomatoRev||0)+(ts.SwiggyRev||0))/tRev*100).toFixed(0)+'%Del':'—';
      var verdict=rd===null?'—':down?(Math.abs(rp)>20?'🔴 Major Drop':'🟠 Moderate Drop'):(rp>15?'🟢 Outperforming':'🔵 On Track');
      var m=s.match(/\(([^)]+)\)/);
      return '<tr>'
        +'<td style="color:'+slotColors[idx]+';font-weight:700">'+s.split(' ')[0]+'<div style="font-size:9px;color:var(--m1);font-weight:400">'+(m?m[1]:'')+'</div></td>'
        +'<td class="num">₹'+fmtN(tRev)+'</td>'
        +'<td class="num">'+fmtN(tOrd)+'</td>'
        +'<td class="num">'+(rRev!==null?'₹'+fmtN(rRev):'—')+'</td>'
        +'<td class="num">'+(rOrd!==null?fmtN(rOrd):'—')+'</td>'
        +'<td class="num" style="color:'+col+';font-weight:700">'+(rd!==null?(rd>=0?'+':'')+'₹'+fmtN(Math.abs(rd)):'—')+'</td>'
        +'<td class="num" style="color:'+(od!==null&&od<0?'var(--red)':od!==null&&od>=0?'var(--grn)':'var(--m1)')+'">'+(od!==null?(od>=0?'+':'')+od.toFixed(0):'—')+'</td>'
  ...
  if(rootCard) {
    ...
    var overallDelta = prevMoSameWdRow?(todayRow.GrossRevenue-prevMoSameWdRow.GrossRevenue):null;
    var overallPct   = (overallDelta!==null&&prevMoSameWdRow.GrossRevenue>0)?(overallDelta/prevMoSameWdRow.GrossRevenue*100):null;
    var isGoodDay    = overallDelta===null||overallDelta>=0;
    var rcHtml = '';
    if(overallDelta!==null){
      rcHtml += '<div style="margin-bottom:10px;padding:10px 14px;border-radius:8px;background:'+(isGoodDay?'rgba(16,185,129,0.08)':'rgba(244,63,94,0.08)')+';border-left:3px solid '+(isGoodDay?'var(--grn)':'var(--red)')+'">'
        ...
        +dayNamesFull[selectedWeekday]+' '+selectedDate+' recorded <strong style="color:var(--txt)">₹'+fmtN(todayRow.GrossRevenue)+'</strong> gross revenue, '
        +(isGoodDay?'up':'down')+' <strong style="color:'+(isGoodDay?'var(--grn)':'var(--red)')+'">'+(overallDelta>=0?'+':'')+'₹'+fmtN(Math.abs(overallDelta))+' ('+(overallPct>=0?'+':'')+overallPct.toFixed(1)+'%)</strong> vs same '+selectedWeekdayShort+' last month.'
        +'</div>';
    } else {
      rcHtml += '<div style="padding:10px 14px;border-radius:8px;background:var(--s2);margin-bottom:10px">No prior month reference. Showing today\'s data only.</div>';
    }
    // Worst channel
    var worstCh=null, worstChDelta=0;
    var chList=[
      {n:'Dine-In',today:todayRow.DineInRev,ref:prevMoSameWdRow?prevMoSameWdRow.DineInRev:null},
      {n:'Pickup',today:todayRow.PickupRev,ref:prevMoSameWdRow?prevMoSameWdRow.PickupRev:null},
      {n:'Delivery',today:todayRow.DeliveryRev,ref:prevMoSameWdRow?prevMoSameWdRow.DeliveryRev:null},
      {n:'Zomato',today:todayRow.ZomatoRev,ref:prevMoSameWdRow?prevMoSameWdRow.ZomatoRev:null},
      {n:'Swiggy',today:todayRow.SwiggyRev,ref:prevMoSameWdRow?prevMoSameWdRow.SwiggyRev:null},
    ];
    chList.forEach(function(c){
      if(c.ref===null) return;
      var d=c.today-c.ref;
      if(d<0&&(worstCh===null||d<worstChDelta)){ worstCh=c; worstChDelta=d; }
    });
    if(worstCh){
      var chPct=worstCh.ref>0?(worstChDelta/worstCh.ref*100):0;
      rcHtml+='🔴 <strong>Primary Channel Drop:</strong> <span style="color:var(--txt)">'+worstCh.n+'</span> fell <strong style="color:var(--red)">₹'+fmtN(Math.abs(worstChDelta))+' ('+chPct.toFixed(1)+'%)</strong> vs reference — highest-impact channel shift.<br>';
    }
    // Worst slot
    var worstSlot=null, worstSlotPct=0;
    slotNames.forEach(function(s){
      var ts=todaySlotRows[s], rs=refSlotRows?refSlotRows[s]:null;
      if(!ts||!rs||rs.GrossRevenue===0) return;
      var p=(ts.GrossRevenue-rs.GrossRevenue)/rs.GrossRevenue*100;
      if(worstSlot===null||p<worstSlotPct){ worstSlot=s; worstSlotPct=p; }
    });
    if(worstSlot&&worstSlotPct<0){
      rcHtml+='🕐 <strong>Underperforming Slot:</strong> <span style="color:var(--txt)">'+worstSlot.split(' ')[0]+'</span> saw <strong style="color:var(--red)">'+worstSlotPct.toFixed(1)+'%</strong> revenue decline. Check ops for that window.<br>';
    }
    if(todayRow.CancelledOrders>0){
      var cPct=todayRow.TotalOrders>0?(todayRow.CancelledOrders/todayRow.TotalOrders*100):0;
      rcHtml+='⚠️ <strong>Cancellations:</strong> <span style="color:var(--red)">'+fmtN(todayRow.CancelledOrders)+' cancelled</span> ('+cPct.toFixed(1)+'% rate). Check platform accept times &amp; kitchen load.<br>';
    }
    var dayAov=todayRow.TotalOrders>0?(todayRow.GrossRevenue/todayRow.TotalOrders):0;
    var refAov=prevMoSameWdRow&&prevMoSameWdRow.TotalOrders>0?(prevMoSameWdRow.GrossRevenue/prevMoSameWdRow.TotalOrders):null;
    if(refAov&&Math.abs(dayAov-refAov)>10){
      rcHtml+='💰 <strong>AOV Shift:</strong> Today ₹'+dayAov.toFixed(0)+' vs ref ₹'+refAov.toFixed(0)+'. '+(dayAov<refAov?'Possible discount overuse or basket shrinkage.':'Strong ticket size — check combo/upsell drivers.');
    }
    rcBody.innerHTML = rcHtml||'<span style="color:var(--m1)">No significant anomalies detected for this day.</span>';
  }
};
...
function getWeekCompareData(allData) {
  var allDay = allData.filter(function(r){ return r.Date && r.MealSlot.toUpperCase().indexOf('ALL DAY') !== -1; });
  if (allDay.length === 0) return null;
  allDay.sort(function(a,b){ return a.Date.localeCompare(b.Date); });
  
  var maxDateStr = allDay[allDay.length - 1].Date;
  var maxD = new Date(maxDateStr);
  
  var curEnd = new Date(maxD);
  var curStart = new Date(maxD);
  ...
  var curStartStr = fmt(curStart), curEndStr = fmt(curEnd);
  var prevStartStr = fmt(prevStart), prevEndStr = fmt(prevEnd);
  
  var curRows = allDay.filter(function(r) { return r.Date >= curStartStr && r.Date <= curEndStr; });
  var prevRows = allDay.filter(function(r) { return r.Date >= prevStartStr && r.Date <= prevEndStr; });
  
  if (curRows.length === 0 || prevRows.length === 0) return null;
  
  var curGross = curRows.reduce(function(a,r){ return a+r.GrossRevenue;}, 0);
  var curOrders = curRows.reduce(function(a,r){ return a+r.TotalOrders;}, 0);
  var tgtGross = prevRows.reduce(function(a,r){ return a+r.GrossRevenue;}, 0);
  var tgtOrders = prevRows.reduce(function(a,r){ return a+r.TotalOrders;}, 0);
  ...
  return {
    ...
    sssg: sssg,
    sstg: sstg,
    daysCompared: curRows.length,
    targetMonthLabel: 'Last 7 Days'
  };
}
...
function buildSalesDetailCharts() {
  ...
  // Always rebuild dropdown from live data so newly synced dates appear immediately
  if (lastOutletId !== activeOutletId) prevMonthVal = '';
  monthFilter.dataset.outletId = activeOutletId;

  var html = '<option value="all">All Months</option>';
  sortedMonths.forEach(function(m) {
    var val = uniqueMonths[m];
    html += '<option value="' + val + '"' + (prevMonthVal === val ? ' selected' : '') + '>' + m + '</option>';
  });
  monthFilter.innerHTML = html;

  // Default to latest month if nothing valid was previously selected
  var validPrev = sortedMonths.some(function(m){ return uniqueMonths[m] === prevMonthVal; });
  if (!validPrev && sortedMonths.length) {
  ...
  if (kpiGrid) {
    kpiGrid.innerHTML = [
      {l:'GROSS REVENUE', v:'₹'+fmtN(totalGross), s:'Total sales before tax & pkg', c:'var(--grn)'},
      {l:'NET REVENUE', v:'₹'+fmtN(totalNet), s:'Net Sales (Gross - Tax - Pkg)', c:'#facc15'},
      ...
      {l:'PACKAGING & TAX', v:'₹'+fmtN(totalPkg + totalTax), s:'Pkg: ₹'+fmtN(totalPkg)+' | Tax: ₹'+fmtN(totalTax), c:'#38bdf8'}
    ].map(function(k){
      return '<div class="kpi-card"><div class="kpi-lbl">'+k.l+'</div>'
            +'<div class="kpi-val" style="color:'+k.c+'">'+k.v+'</div>'
            +'<div class="kpi-sub">'+k.s+'</div></div>';
  ...
  if (growthGrid) {
    growthGrid.className = 'grid3';
    var wowData = getWeekCompareData(data);
    ...
    var renderGrowthCard = function(title, gData) {
      ...
      var footerTxt = title.indexOf('Week') !== -1 ? (gData.daysCompared + ' Days Compared') : ('Matched Day Range: Days 1-' + gData.daysCompared);
      ...
      return '<div style="background:var(--s2); border:1px solid var(--b1); border-radius:12px; padding:16px">'
        ...
        + '</div>'
        + '<div style="font-size:9px; color:var(--m1); margin-top:10px; border-top:1px solid var(--b1); padding-top:6px">' + footerTxt + '</div>'
        + '</div>';
    };
    
    if (selectedMonth === 'all') {
      growthGrid.innerHTML = renderGrowthCard('Week-over-Week (WoW)', wowData) + '<div style="grid-column: span 2; text-align:center; padding:16px; color:var(--m1); font-size:11px">Select a specific month to view MoM/YoY growth.</div>';
    } else {
      growthGrid.innerHTML = renderGrowthCard('Week-over-Week (WoW)', wowData) + renderGrowthCard('Month-over-Month (MoM)', momData) + renderGrowthCard('Year-over-Year (YoY)', yoyData);
    }
  }
        ...
        options: {
          ...
          plugins: {
            ...
            datalabels: {
              ...
              anchor: 'end',
              align: 'top',
              font: { weight: 'bold', size: 10, family: "'DM Sans', sans-serif" },
              formatter: function(val) { return val > 0 ? fmtN(val) : ''; }
            },
            tooltip: {
              ...TT,
              callbacks: {
